  ...
  return (
    <footer className="bg-gray-900 text-gray-300 pt-12 pb-6">
          ...
          <div className="flex gap-3">
            {[Facebook, Twitter, Instagram, Youtube].map((Icon, i) => (
              <a
                key={i}
                href="#"
                className="w-9 h-9 bg-gray-700 rounded-full flex items-center justify-center hover:bg-orange-500 transition"
              >
                <Icon size={16} />
              </a>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm">
            {[
              ...
              <li key={path}>
                <Link to={path} className="hover:text-orange-500 transition">
                  {label}
                </Link>
              </li>
            ))}
      ...
      <div className="border-t border-gray-700 mt-10 pt-6 text-center text-sm text-gray-500">
        © {new Date().getFullYear()} ssdealsify. All rights reserved.
      </div>
    </footer>
  );
}
